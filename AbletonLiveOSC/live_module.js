...
const myParameters = {};
const lbl = {};
// MUDANÇA: Usaremos targets definidos no config
const host = '127.0.0.1';
const port = 11000;
const names = {};
const track_names = []; // Agora vamos tentar preencher isso
const selno = 0;
const hex_colors_id = [
...
// Helper para enviar Logs para o Python E para o Console Interno
function logBoth(msg) {
	receive('/LOG', msg);
	send('127.0.0.1', 1337, '/log', msg);
}
...
module.exports = {
	...
	oscInFilter: function (data) {
		...
		logBoth(`MSG IN: ${data.address}, ${data.args.map(a => a.value).join(', ')}`)
	...
	oscOutFilter: function (data) {
		...
		logBoth(`MSG OUT: ${data.address}, ${data.args.map(a => a.value).join(', ')}`)
		...
		// ==========================================================
		// PATCH ANTIGRAVITY v3 (PYTHON BRIDGE)
		// Intercepta /note e envia para o servidor Python na porta 1337.
		// O Python (server.py) se encarrega de mandar MIDI para o LoopMIDI.
		// ==========================================================
		if (address == '/note') {
			var channel = data.args[0].value;
			var note = data.args[1].value;
			var velocity = data.args[2].value;
			velocity = Math.round(velocity);

			// Envia para o servidor Python
			// send(host, port, address, ...args)
			// Target: 127.0.0.1, Porta: 1337, Address: /note

			logBoth(`>> BRIDGE PYTHON: Ch:${channel} Note:${note} Vel:${velocity} -> 1337`);

			// A função send do OSC JS pode variar dependendo da versão, mas geralmente:
			// send(address, args...) vai para o target padrão
			// Aqui vamos usar o target específico

			send('127.0.0.1', 1337, '/note', channel, note, velocity);

			// Bloqueia envio duplicado
			return null;
		}
		// ==========================================================
